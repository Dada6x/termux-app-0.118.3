...
public final class TerminalView extends View {
    ...
    /** On Wear OS, the typed command line is kept here so it survives a keyboard close/open cycle. */
    private String mWatchBufferedText = "";
 ...
 * the extra keys row on demand. One notification per scroll direction change. */
    ...
    private WatchTerminalScrollListener mWatchTerminalScrollListener;

    /** Distance dragged upward since the direction changed, in pixels. */
    private float mWatchScrollDistance = 0f;
    ...
    public void setWatchTerminalScrollListener(WatchTerminalScrollListener watchTerminalScrollListener) {
        mWatchTerminalScrollListener = watchTerminalScrollListener;
    }
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        if (mClient.isTerminalViewSelected()) {
            if (isWatchDevice(getContext())) {
                // Wear OS keyboards (WearGboard) interpret TYPE_NULL and VISIBLE_PASSWORD as a
                // password field, load keyboard_password layout, fail to resolve it, and close
                // the keyboard immediately. Use a short message (SMS) style text field: like other
                // messaging apps it keeps a live command line in the keyboard and may offer a
                // dedicated enter/newline key, while the typed text stays in the input so the Wear
                // keyboard can render, edit and delete it in a preview strip.
                outAttrs.inputType = InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_SHORT_MESSAGE
                    | InputType.TYPE_TEXT_FLAG_IME_MULTI_LINE;
                // SMS style input may auto-capitalize the first letter; keep the Wear keyboard
                // in its lowercase state when it opens.
        ...
        if (isWatchDevice(getContext())) {
            // WearGboard has no built-in enter/newline key on its fixed layouts, but it renders an
            // action key when the field explicitly requests one. The action is translated to a
            // newline by the performEditorAction() override in the input connection. Stay in
            // fullscreen mode since the Wear non-fullscreen keyboard deactivates in Termux.
            outAttrs.imeOptions = EditorInfo.IME_ACTION_SEND;
            outAttrs.initialSelStart = mWatchBufferedText.length();
            outAttrs.initialSelEnd = mWatchBufferedText.length();
        } else {
            outAttrs.imeOptions = EditorInfo.IME_FLAG_NO_FULLSCREEN;
        }

        TermuxInputConnection connection = new TermuxInputConnection(this, true);
        if (isWatchDevice(getContext()))
            mWatchInputConnection = connection;
        return connection;
    }

    /**
     * The {@link InputConnection} of {@link TerminalView}. On phones it keeps the original
     * flush-each-commit behaviour. On Wear OS the input behaves as a command line: the typed text
     * stays in the input so the Wear keyboard can render, edit and delete it live (single source
     * of truth). It is flushed to the terminal only when a newline (enter) is sent. The text is
     * mirrored to {@code mWatchBufferedText} so closing/reopening the keyboard does not lose it.
     */
    private final class TermuxInputConnection extends BaseInputConnection {

        TermuxInputConnection(View targetView, boolean fullEditor) {
            super(targetView, fullEditor);
        }

        boolean mWatchSeeded = false;

        /** Token WearGboard registers with, used to push live extracted text while typing. */
        int mWatchExtractionMonitorToken = -1;

            private void ensureWatchSeed() {
                if (!mWatchSeeded) {
                    mWatchSeeded = true;
                    if (mWatchBufferedText != null && mWatchBufferedText.length() > 0)
                        getEditable().append(mWatchBufferedText);
                }
            }

            private void persistWatchBuffer() {
                if (isWatchDevice(getContext()))
                    mWatchBufferedText = getEditable().toString();
            }

            private void notifyWatchExtractionChanged() {
                if (!isWatchDevice(getContext())) return;
                InputMethodManager imm = (InputMethodManager) getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
                if (imm == null) return;
                Editable editable = getEditable();
                int sel = editable.length();
                // Make the Wear keyboard re-read the extracted text so its preview strip updates
                // without needing to close/reopen the keyboard. updateSelection() mirrors what
                // EditText does and makes the IME repull the text; when the IME registered a
                // monitor token we additionally push the text directly.
                imm.updateSelection(TerminalView.this, sel, sel, -1, -1);
                if (mWatchExtractionMonitorToken != -1) {
                    ExtractedText extractedText = buildExtractedText(editable);
                    imm.updateExtractedText(TerminalView.this, mWatchExtractionMonitorToken, extractedText);
                }
            }

            private ExtractedText buildExtractedText(CharSequence text) {
                int len = text.length();
                ...
                extractedText.selectionStart = len;
                extractedText.selectionEnd = len;
                extractedText.flags = 0;
                return extractedText;
            }

            /** Called by {@link #onKeyDown(int, KeyEvent)} when the Wear keyboard backspace key is pressed. */
            private boolean deleteLastCharOfWatchBuffer() {
                ensureWatchSeed();
                Editable editable = getEditable();
            ...
            private void flushWatchBufferToTerminal() {
            ...
            public ExtractedText getExtractedText(ExtractedTextRequest request, int flags) {
                // WearGboard needs a non-null extracted text matching the typed text, otherwise it
                // can not show the preview strip and even deactivates the keyboard. On phones keep
                // the default (null) BaseInputConnection behaviour.
                if (isWatchDevice(getContext())) {
                    ensureWatchSeed();
                    CharSequence text = getEditable();
                    // Remember the monitor token WearGboard registers with so we can push live
                    // updates to its preview strip while the user types.
                    if ((flags & InputConnection.GET_EXTRACTED_TEXT_MONITOR) != 0 && request != null)
                        mWatchExtractionMonitorToken = request.token;
                    return buildExtractedText(text);
                }
            ...
            public boolean performEditorAction(int editorAction) {
                ...
                if (isWatchDevice(getContext())) {
                    // Send the buffered command line to the terminal followed by a newline so the
                    // shell executes it, like pressing enter.
                    ensureWatchSeed();
                    flushWatchBufferToTerminal();
            ...
            public boolean setComposingText(CharSequence text, int newCursorPosition) {
                ...
                if (mEmulator == null) return result;

                if (isWatchDevice(getContext())) {
                    // WearGboard writes individual key presses as composing text and only commits
                    // (refreshing the preview strip) on space or swipe gestures. For a reactive
                    // command line the composing text must be mirrored into the editable and pushed
                    // live on every key press, without flushing anything to the terminal.
                    ensureWatchSeed();
                    persistWatchBuffer();
                    notifyWatchExtractionChanged();
                }
            ...
            public boolean commitText(CharSequence text, int newCursorPosition) {
                ...
                if (isWatchDevice(getContext())) {
                    ensureWatchSeed();
                    if (containsNewline(getEditable())) {
                        flushWatchBufferToTerminal();
                    } else {
                        persistWatchBuffer();
                    }
    ...
    /**
     * Check if the device is a Wear OS watch or not, same as {@link com.termux.shared.android.DeviceUtils#isWatchDevice}.
     */
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_DEL && isWatchDevice(getContext())) {
            // On Wear OS the typed command line lives in the input buffer, not the terminal, so the
            // keyboard's backspace key must edit the buffer instead of sending DEL to the shell.
            if (mWatchInputConnection != null && mWatchInputConnection.deleteLastCharOfWatchBuffer()) {
                invalidate();
                return true;
            }
        }

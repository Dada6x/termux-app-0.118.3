...
public class KeyboardUtils {
    ...
    /**
     * Show the soft keyboard forcefully for the {@code view}. {@link InputMethodManager#SHOW_FORCED}
     * is passed as {@code showFlags} so the keyboard is shown even if the current soft keyboard app
     * overrides the default implementation of {@link InputMethodService#onEvaluateInputViewShown()}
     * and returns {@code false}, for example when a hardware keyboard is connected. This is the
     * same flag used by {@link #toggleSoftKeyboard(Context)}.
     *
     * @param context The {@link Context} of the {@code view}.
     * @param view The {@link View} for which the soft keyboard should be shown.
     */
    public static void showSoftKeyboardForced(final Context context, final View view) {
        if (context == null || view == null) return;
        InputMethodManager inputMethodManager = (InputMethodManager) context.getSystemService(Context.INPUT_METHOD_SERVICE);
        if (inputMethodManager != null)

...
public abstract class TermuxSharedProperties {
    ...
    public static Object getInternalTermuxPropertyValueFromValue(Context context, String key, String value) {
        ...
        switch (key) {
            ...
            case TermuxPropertyConstants.KEY_EXTRA_KEYS:
                return (String) getExtraKeysInternalPropertyValueFromValue(context, value);
    ...
    /**
      * Returns the value itself if it is not {@code null}, otherwise returns the default value for
      * the device, which is {@link TermuxPropertyConstants#DEFAULT_IVALUE_EXTRA_KEYS} on phones,
      * tablets and other devices and {@link TermuxPropertyConstants#DEFAULT_IVALUE_EXTRA_KEYS_WATCH}
      * on Wear OS (watch) devices so the keys fit the small watch screen.
      *
      * @param context The {@link Context} to check the device type with.
      * @param value The {@link String} value to convert.
      * @return Returns the internal value for value.
      */
    public static String getExtraKeysInternalPropertyValueFromValue(@NonNull Context context, String value) {
        String defaultExtraKeys = DeviceUtils.isWatchDevice(context) ?
            TermuxPropertyConstants.DEFAULT_IVALUE_EXTRA_KEYS_WATCH : TermuxPropertyConstants.DEFAULT_IVALUE_EXTRA_KEYS;
        return SharedProperties.getDefaultIfNullOrEmpty(value, defaultExtraKeys);
    }
